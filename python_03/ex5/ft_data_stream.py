...
# Iterator
# [1] Object used to iterate over
# Iterable using next() method Return 1 element ar a time
...
# [3] By using next() it resume from where it called “yield” not from beginning


# for storing all analytics in an object type analytics
class analytics:
    def __init__(
        self,
        total_events=0,
        ...
        self.high_level_players = high_level_players
    ...
    def display_stats(self):
        print("\n=== Stream Analytics ===\n")
        print("Total events processed: ", self.total_events)
        print("High-level players (10+): ", self.high_level_players)
        print("kill events: ", self.kill_event)
        print("Level-up events: ", self.level_up_event)


def gen_message_analy(data: list[dict[str]], stat: analytics):
    """
    this is a generator function that pause its execution
    using the keyword yield and it continue when using the method next()
    """
    total_events = 0
    high_level_players = 0
    kill_event = 0
    ...
    for my_data in iter(data):
        player_name = my_data["player"]
        type_event = my_data["event_type"]
        if type_event == "kill":
            kill_event += 1
        elif type_event == "level_up":
            level_up_event += 1
        level = my_data["data"]["level"]
        if level >= 10:
            high_level_players += 1
        message = f"Player {player_name} (level {level}) {type_event} "
...
# this generator function return an item in the fibonnaci list
...
# this function print a fibonnaci list


def fib_list(pos):
    if pos < 0:
        raise ValueError("the position is negative")
    my_gen = fibonnaci(pos)
...
if __name__ == "__main__":
    ...
    print(f"Processing {len(data_list)} game events...\n")
    stats = analytics()

    gen = gen_message_analy(data_list, stats)
    ...
    try:
        ...
        print("Type of Erreur: StopIteration")
    ...
    try:
        ...
        print(f"Fibonacci sequence (first {pos}) : ", end="")
        fib_list(pos)

    except ValueError as e:
        print("Error : ", e)

    gen_prime = prime_list(5)

    print("Prime numbers (first 5): ", end="")
    for nbr in gen_prime:
        print(nbr, end=" ")
